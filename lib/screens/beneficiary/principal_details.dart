...
class PrincipalDetailsScreen extends StatefulWidget {
  int? index;
  PrincipalDetailsScreen({Key? key, this.index}) : super(key: key);
    ...
    if (!(state?.currentPlanData!['isSponsor'] ?? true)) {
      if (!state!.isLoggedIn) {
        ...
        _firstNameController.text =
            "${AvonData().avonData.get('firstName${widget.index}') ?? ""}";
        _lastNameController.text =
            "${AvonData().avonData.get('lastName${widget.index}') ?? ""}";
        _emailController.text =
            "mainEmail${AvonData().avonData.get('email${widget.index}') ?? ""}";
        title =
            "${AvonData().avonData.get('title${widget.index}') ?? "Select"}";
        gender =
            "${AvonData().avonData.get('gender${widget.index}') ?? "Select"}"; // == "MALE" ? "Male" : "Female";
        maritalStatus =
            "${AvonData().avonData.get('maritalStatus${widget.index}') ?? "Select"}"; // state?.plan!.maritalStatus ?? "Single";
        _dobController.text =
            "${AvonData().avonData.get('dob${widget.index}') ?? ""}";
        // "${formatter.format(DateTime.parse("${state?.plan?.dob}"))}";
        _emailController.text =
            "${AvonData().avonData.get('email${widget.index}') ?? ""}";
        _sponsorPhoneController.text =
            "${AvonData().avonData.get('sponsorPhone${widget.index}') ?? ""}";
      ...
      // _emailController.text = "${AvonData().avonData.get('email${widget.index}')}";
      // _emailController.text = "${state?.user.email}";
      _sponsorPhoneController.text =
          "${AvonData().avonData.get('sponsorPhone${widget.index}')}";
      // _sponsorPhoneController.text = "${state?.user.mobilePhone ?? ""}";
      _sponsorEmailController.text =
          "${AvonData().avonData.get('sponsorEmail${widget.index}')}";
      // _sponsorEmailController.text = "${data['CREATEDBY']}";
      _phoneController.text =
          "${AvonData().avonData.get('phoneNumber${widget.index}')}";
    }
    // } catch (e) {
                  ...
                  children: [
                    ...
                    Column(
                      children: [
                        ...
                        SizedBox(height: 10),
                        AVDropdown(
                          options: ["Select", "Married", "Single", "Divorce"],
                          value: maritalStatus,
  ...
  submit() async {
    ...
    Map payload = {
      ...
      "PHONENUMBER": _phoneController.text.isNotEmpty
          ...
          : AvonData().avonData.get('phoneNumber') ?? "" // for sponsor
    };
    ...
    AvonData().avonData.putAll({
      'paymentUser': _firstNameController.text.toString(),
      'phoneNumber${progress - 1}': _phoneController.text.toString(),
      'sponsorPhone${progress - 1}': _sponsorPhoneController.text.toString(),
      'sponsorEmail${progress - 1}': _sponsorEmailController.text.toString(),
      'gender${progress - 1}': gender.toString(),
      'maritalStatus${progress - 1}': maritalStatus.toString(),
      'firstName${progress - 1}': _firstNameController.text.toString(),
      'lastName${progress - 1}': _lastNameController.text.toString(),
      'dob${progress - 1}': _dobController.text.toString()
    });
