...
class _PaymentOptionState extends State<PaymentOption> {
  ...
  handleResponse(value) async {
    ...
    if (value['message'] == 'Successful') {
      ...
      AvonData().avonData.put('paymentUser', data['firstName']);
      ...
      if (response.statusCode == 200) {
        ...
        if (data['hasError'] == false) {
          ...
          String paymentUser = "";
          paymentUser = AvonData().avonData.get('paymentUser');
          if (!state.isLoggedIn && state.isLastPlan) {
            state.cartData.clear();
            state.cart.clear();
            Navigator.pushReplacement(context, MaterialPageRoute(builder: (_)=>BuyPlanSuccess(navigateTo: 'welcome',)));
            return;
          } else {
            showAlertDialog(
                context: context,
          ...
          if (!state.currentPlanData!['isSponsor']) {
            ...
            if (state.isLastPlan) {
              ...
              Navigator.pushReplacement(context,
                  MaterialPageRoute(builder: (context) => BuyPlanSuccess(firstName: paymentUser,)));
            } else {
              print("2222");
              showAlertDialog(
                  context: context,
